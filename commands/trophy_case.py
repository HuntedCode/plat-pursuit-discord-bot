...
class TrophyCaseCog(commands.Cog):
    ...
    def _build_view(self, current_page:int, total_pages: int):
        ...
        async def prev_callback(interaction: discord.Interaction):
            ...
            if current_page > 1:
                current_page -= 1
                new_data = await self.fetch_page(interaction.user.id, current_page)
                if new_data:
                    new_embeds = await self.build_embeds(new_data)
                    new_view = self._build_view(current_page, new_data['total_pages'])
                    await interaction.response.edit_message(embeds=new_embeds, view=view)
                else:
                    await interaction.response.edit_message(content='Error fetching page.', embeds=None, view=new_view)
        ...
        async def publish_callback(interaction: discord.Interaction):
            try:
                await interaction.channel.send(embeds=await self.build_embeds(await self.fetch_page(interaction.user.id, current_page)))
    ...
    async def fetch_page(self, discord_id, current_page, per_page=4):
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(f"{self.bot.api_base_url}trophy-case/", params=params, headers=headers) as resp:
                if resp.status == 200:
    ...
    async def build_embeds(self, page_data):
        ...
        if not platinums:
            ...
            for index, p in enumerate(platinums):
                #embed = discord.Embed(title=p['trophy_name'], color=0xFFD700)
                if index == 0:
                    embed = discord.Embed(title=f"Your Digital Trophy Case! {self.bot.plat_pursuit_emoji} {self.bot.platinum_emoji}", color=0xFFD700, url='https://www.google.com/')
                    embed.set_footer(text=f"Page {page_data['current_page']}/{page_data['total_pages']} | Total Platinums: {page_data['total_plats']}")
                else:
                    embed = discord.Embed(url='https://www.google.com/')
                if p['icon_url']:
