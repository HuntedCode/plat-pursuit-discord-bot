...
class SummaryCog(commands.Cog):
    ...
    async def trophies(self, interaction: discord.Interaction, user: discord.Member = None):
        ...
        async with session.get(f"{self.bot.api_base_url}summary/", params=params) as resp:
            if resp.status == 200:
                ...
                if data.get('linked'):
                    ...
                    await interaction.followup.send('No PSN linked. Use /link first!', ephemeral=True)
            else:
